    ...
    for x in commands:
        ...
        if task == 'basename':
            ...
            if base == False:
                ...
                print "You did not specify a basename."
                num_frames = x['args'][0]
                frames = True
        elif task == 'vary':
            animation = True
            if frames == False:
                raise Exception("Invalid frame rate value.")
    return animation
...
def second_pass( commands):
    ...
    for x in commands:
        ...
        if name == 'vary':
            ...
            startvalue = args[2]
            endvalue = args[3]
            delta = (endvalue - startvalue) / (endframe - startframe)
            a = startvalue
            for y in range(int(startframe), int(endframe + 1)):
                if y == endframe:
                        a = endvalue
                framelist[y][knobval] = a
                a += delta
    return framelist
